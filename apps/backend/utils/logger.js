...
// Specialized logger for match events
const matchesLogger = winston.createLogger({
	level: 'info',
	format: logFormat,
	defaultMeta: { service: 'face-recognition' },
	transports: [
		new winston.transports.File({
			filename: path.join(logDirPath, 'matches.log'),
		}),
	],
});
...
// Log match events in the structured format
const logMatch = (name, timestamp) => {
	const logEntry = {
